...
def addTest(request,subId):
    ...
    if request.method=="GET":
        ...
        subj = Subject.objects.filter(id=subId).all()[0]
        test = SubjTest.objects.create(subj=subj, name=request.POST["name"], questions_num=request.POST["questions_num"])
        test.save()
...
def detailsTest(request, subId, testId):
    ...
    if not request.user.is_staff:
        usertest = UserTest.objects.filter(user=request.user,test=test,isFinished=False).all()
        if not usertest:
            usertest = UserTest.objects.create(test=test,user=request.user,time_start=datetime.now().isoformat())
            usertest.save()
            for i in questions:
                opts = Option.objects.filter(quest=i).all()
                for opt in opts:
                    UserAnswer.objects.create(quest=i, test=usertest,user=request.user,opt=opt,answer=False).save()
        
    if not request.user.is_authenticated:
        return redirect("logauth:log")

    if len(questions) < test.questions_num:
        for i in range(test.questions_num - len(questions)):
    ...
    if request.method == "GET":
        ...
        if request.user.is_staff:
            ...
            while len(opts) < int(question.optionsNum):
                ...
                opts = Option.objects.filter(quest=question).all()
            question.save()
            return redirect("testApp:changeOpts", subId=subId, testId=testId, questId=questId)
        else:
            ####DO
            test = UserTest.objects.filter(test=SubjTest.objects.filter(id=testId).all()[0],user=request.user,isFinished=0)[0]
            question = Question.objects.filter(id=questId).all()[0]
            opts = Option.objects.filter(quest=question).all()
            for opt in opts:
                answer = UserAnswer.objects.filter(test=test,quest=questId, user=request.user, opt=opt).all()
                if not(answer):
                    answer = UserAnswer.objects.create(test=test,quest=question,user=request.user,opt=opt)
                    answer.save()
...
def finishTest(request,testId):
    test = UserTest.objects.filter(test=testId, user=request.user,isFinished=False).all()[0]
    ...
    for question in questions:
        ...
        for user_ans in UserAnswer.objects.filter(user=request.user,test=test,quest=question).all():
            print(user_ans)
            print(user_ans.answer, user_ans.opt.isRight, 1)
...
def user_stat(request):
    if not request.user.is_staff:
        user_tests = UserTest.objects.filter(user=request.user, isFinished=True).all()[::-1]
    else:
    ...
    context = {
        "tests" : list(zip(user_tests,max_scores))
    }
