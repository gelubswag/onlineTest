...
from testApp.forms import SubjectForm, TestForm, QuesttForm, OptForm, StartTestForm
...
@decorators.login_required
def run_test(request, subId, testId):
    test = SubjTest.objects.filter(id=testId).all()[0]
    if request.method == 'GET':
        context = {"pageTitle": f"Введите количество вопросов (макс. {test.questions_num})", "subId": subId, "testId": testId, 'form': StartTestForm()}
        return render(request, 'test/testStartForm.html', context)
    else:
        test.test_questions_num = request.POST["test_questions_num"]
        test.save()
        return redirect('testApp:detailsTest', subId=subId, testId=testId)
...
def detailsQuest(request, subId, testId, questId):
    if request.method == "GET":
        ...
        if not (request.user.is_staff):
            ...
            context["user_questions"] = enumerate([
                Question.objects.filter(id=i).all()[0] for i in UserTest.objects.filter(test=test, user=request.user, isFinished=False).all()[0].question_pool
                ])
            if not (questId in UserTest.objects.filter(test=test, user=request.user, isFinished=False).all()[0].question_pool):
                print(questId)
                return redirect("testApp:main")
            ...
            if questId == usertest.question_pool[0]:
        ...
        if request.user.is_staff:
            ...
            question.imageHeight = request.POST["imageHeight"]
            question.imageWidth = request.POST["imageWidth"]
            opts = Option.objects.filter(quest=question).all()
            if len(opts) > int(question.optionsNum):
                Option.objects.filter(quest=question).delete()
...
@staff_member_required
def addQuest(request, subId, testId):
    context = {"pageTitle": "Добавление вопроса", "subId": subId, "testId": testId, 'form': QuesttForm()}
    if request.method == 'GET':
        return render(request, "test/subjectForm.html", context)
    else:
        data = request.POST
        form = QuesttForm(data)
        if form.is_valid():
            quest = form.save(commit=False)
            quest.test = SubjTest.objects.filter(id=testId).all()[0]
            quest.test.questions_num += 1
            quest.test.save()
            quest.save()
            return redirect("testApp:detailsQuest", subId=subId, testId=testId, questId=quest.id)
@staff_member_required
...
def deleteQuest(request, subId, testId, questId):
    question = Question.objects.filter(id=questId).all()[0]
    question.delete()
    question.test.questions_num -= 1
    question.test.save()
    return redirect("testApp:detailsTest", subId=subId, testId=testId)
...
def changeOpts(request, subId, testId, questId):
    if request.method == "GET":
        ...
        for i in ids:
            ...
            imageHeight = request.POST.get(f"imageHeight{i}")
            imageWidth = request.POST.get(f"imageWidth{i}")
...
def user_stat(request):
    ...
    for test in user_tests:
        questions = test.question_pool
        max_score = 0
        for question in [Question.objects.filter(id=question_id).all()[0] for question_id in questions]:
            question_right = question.weight
            max_score += question_right
