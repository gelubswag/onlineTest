...
class SubjTest(models.Model):
    ...
    questions_num = models.IntegerField(verbose_name="Количество вопросов", validators=[MinValueValidator(1)], default=1)
...
class UserTest(models.Model):
    ...
    def __str__(self):
        return ["Not Finished ", "Finished "][self.isFinished] + self.user.username + "> " + self.test.subj.name + "> " + self.test.name
    
class UserAnswer(models.Model):
    user = models.ForeignKey(User, verbose_name="Пользователь", on_delete=models.CASCADE,null=True, blank=True)
    test = models.ForeignKey(UserTest, verbose_name="Тест", on_delete=models.CASCADE)
    quest = models.ForeignKey(Question, verbose_name="Вопрос", on_delete=models.CASCADE,null=True, blank=True)
    opt = models.ForeignKey(Option, verbose_name="Вариант ответа", on_delete=models.CASCADE,null=True, blank=True)
